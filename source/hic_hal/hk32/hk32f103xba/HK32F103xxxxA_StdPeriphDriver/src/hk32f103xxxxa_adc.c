...
* @filename:  hk32f103xxxxa_adc.c
* @brief:     ADC init and functions configure
* @author:    AE Team
* @version:   V1.0.0/2023-01-15
*             1.Initial version
* @log:
*******************************************************************************/
...
/** @defgroup ADC_Private_Defines ADC_Private_Defines
  ...
  */
/* ADC AWDWAKEEN mask */
#define CR1_AWDWAKE_Set             ((uint32_t)0x80000000)
#define CR1_AWDWAKE_Reset           ((uint32_t)0x7FFFFFFF)
...
/**
  * @brief  Enables or disables the analog watchdog wakeup single/all regular
  *         or injected channels
  * @note   1. this function only used on regular ADC mode
  *         2. must enable AWDEN bit
  *         3. cannot used on cycle and disc mode
  * @param  ADCx: where x can be 1, 2 or 3 to select the ADC peripheral.
  * @param  NewState: new state of the temperature sensor.
  *         This parameter can be: ENABLE or DISABLE.
  * @retval None
...
void ADC_AnalogWatchdogWakeEn(ADC_TypeDef* ADCx, FunctionalState NewState)
{
    ...
    if (NewState != DISABLE)
    {
        /* Enable the AWD wake up stop mode*/
        ADCx->CR1 |= CR1_AWDWAKE_Set;
    }
    else
    {
        /* Disable the AWD wake up stop mode*/
        ADCx->CR1 &= CR1_AWDWAKE_Reset;
    }
}
/**
  * @}
